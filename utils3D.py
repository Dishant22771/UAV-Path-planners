...
def lineSphere(p0, p1, ball):
    ...
    line_magnitude = line[0] * line[0] + line[1] * line[1] + line[2] * line[2]
    if line_magnitude == 0:
        return False
    ...
    class rrt_demo:
        ...
        def run(self):
            while self.ind < self.maxiter:
                ...
                nearest_node = tree_nearest(self.head, xrand) 
                xnew = tree_steer(self, nearest_node, xrand) 
                collide, _ = isCollide(self, nearest_node.pos, xnew) 
                if not collide:
                    new_node = tree_add_edge(nearest_node, xnew) 
